# jukebox.py
# Students Name: Jesse Burden, Sebastian Campos, Quinton Stepps
...
class Library(LinkedList):
    ...
    def build_library(self):
        ...
        self.add(Song("Mayonaka no Door", "Tanaka Yuri", "5:10"))
...
class HtmlAdapter:
    ...
    def to_html(self):
        ...
        for song in self.song_list:
            ...
            playlist = Playlist(self.song_list)

            song_html += f'<div class="song">' \
                         f'<h2>{song.title}</h2>' \
                         f'<h3>{song.artist}</h3>' \
                         f'<p class="html">{playlist}</p>' \
                         f'<p class="json"><!--\n{json_payload}\n--></p>' \
                         f'<p class="xml"><!--\n{xml_payload}\n--></p>' \
                         f'</div>'
        html = f'<html>' \
...
class Playlist:
    ...
    def render_html(self):
        return self.html_adapter.render(self.songs)
